...
export USE_DIAGNOSTIC_DOMAIN_FILTERS="${USE_DIAGNOSTIC_DOMAIN_FILTERS:-0}"
...
if [[ -n "${FORCE_START_INDEX}" ]]; then
  if [[ "${FORCE_START_INDEX}" =~ ^[0-9]+$ ]] && (( FORCE_START_INDEX >= 1 && FORCE_START_INDEX <= TOTAL )); then
    START_INDEX="${FORCE_START_INDEX}"
  else
    echo "Invalid FORCE_START_INDEX=${FORCE_START_INDEX}. Expected integer in [1, ${TOTAL}]." >&2
    exit 1
  fi
fi

if (( START_INDEX > TOTAL )) && [[ "${AUTO_RESTART_ON_COMPLETED}" == "1" ]]; then
  START_INDEX=1
  echo "Previous state indicates pipeline already completed; AUTO_RESTART_ON_COMPLETED=1 so restarting from stage 1."
fi
